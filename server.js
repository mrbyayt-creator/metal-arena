...
function chooseEnemyWeapon(e, len){
  const sp=enemySpecialWeapon(e);
  let chance=e.bossType==='artillery' ? 0.62 : e.boss ? 0.42 : e.role==='shooter' ? 0.30 : 0.22;
  if (sp==='mine') chance = len<28 ? 0.34 : 0.05;
  else if (sp==='shotgun') chance = len<38 ? 0.34 : 0.10;
  else if (sp==='tesla') chance = len<24 ? 0.40 : 0.00;
  else if (sp==='railgun') chance = len>18 ? chance : 0.08;
...
function tickSurvival(lobbyId){
  ...
  for (const e of lobby.enemies.values()){
    ...
    if (canShoot){
      const w=chooseEnemyWeapon(e, len);
      ...
      // Серверный урон меньше и зависит от роли/оружия, чтобы новая спец-пушка не была жёстче старых ракет.
