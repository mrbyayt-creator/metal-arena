...
function removeFromLobby(sock){
  ...
  client.lobbyId=null; client.team=null; client.alive=true; client.deathHandled=false; client.activeInWave=false; client.waitingNextWave=false; client.waitingNextRound=false; resetClientStats(client);
  if (!lobby) return;
...
function startRound(lobby){
  ...
  for (const s of lobby.members){ const c=clients.get(s); if(!c) continue; c.alive=true; c.deathHandled=false; c.waitingNextRound=false; c.hp=1; }
...
function registerPlayerDeath(lobby, dead, killerId){
  if (!lobby || !dead || dead.deathHandled) return false;
  dead.deathHandled=true;
  dead.deaths++;
  dead.alive=false;
  dead.hp=0;
  if (lobby.mode==='survival'){
    dead.waitingNextWave=true;
    lobbyBroadcast(lobby,{ type:'death', id:dead.id, killer:killerId });
    lobbyBroadcast(lobby, scorePayload(lobby));
    checkSurvivalPlayers(lobby);
    return true;
  }
  const killer = killerId!=null ? clients.get(socketById(killerId)) : null;
  if (killer && killer.lobbyId===lobby.id && killer.id!==dead.id && killer.team!==dead.team) killer.kills++;
  lobbyBroadcast(lobby,{ type:'death', id:dead.id, killer:killerId });
  lobbyBroadcast(lobby, scorePayload(lobby));
...
const SERVER_WALLS = [
  // Main cover / containers mirrored from the client arenas so survival bots path around the same major objects.
  { x: 62,  z:-38, hw:18.4, hd:1.7 }, { x:-60,  z: 42, hw:1.7,  hd:18.4 }, { x: 44,  z: 58, hw:18.4, hd:1.7 },
  { x:-52,  z:-54, hw:1.7,  hd:18.4 }, { x:  6,  z:-22, hw:18.4, hd:1.7 },
  { x:-158,z:  8, hw: 7, hd:5 }, { x:158, z:-26, hw:7, hd:5 }, { x:-128,z:86, hw:6, hd:6 }, { x:126,z:92, hw:6, hd:6 },
  { x:-192,z:-96, hw:6, hd:5 }, { x:192,z:86, hw:6, hd:5 }, { x:8,z:-128, hw:9, hd:2.2 }, { x:-8,z:128, hw:9, hd:2.2 },
  { x:-96,z:-118, hw:5, hd:5 }, { x:96,z:118, hw:5, hd:5 }, { x:176,z:-104, hw:5, hd:5 }, { x:-176,z:104, hw:5, hd:5 },
  { x:-142,z:-36, hw:4, hd:10 }, { x:142,z:36, hw:4, hd:10 }, { x:-122,z:-70, hw:12, hd:2.4 }, { x:122,z:70, hw:12, hd:2.4 },
  { x:-72,z:126, hw:12, hd:2.6 }, { x:72,z:-126, hw:12, hd:2.6 }, { x:-214,z:0, hw:4, hd:12 }, { x:214,z:0, hw:4, hd:12 },
  { x:-102,z:18, hw:3.8, hd:3.8 }, { x:104,z:-18, hw:3.8, hd:3.8 }, { x:-132,z:36, hw:8, hd:1.8 }, { x:132,z:-36, hw:8, hd:1.8 },
  { x:-36,z:-132, hw:1.8, hd:8 }, { x:36,z:132, hw:1.8, hd:8 }, { x:-88,z:-88, hw:4.6, hd:3.2 }, { x:88,z:88, hw:4.6, hd:3.2 }
];
...
function startSurvival(lobby){
  ...
  for (const s of lobby.members){
    const c=clients.get(s);
    c.kills=0; c.deaths=0; c.alive=true; c.deathHandled=false; c.team=null; c.activeInWave=true; c.waitingNextWave=false; c.hp=1;
  }
...
// Network-survival bot weapons: same idea as the client campaign/survival bots.
// Only bot weapon choice/balance is affected; player weapon values are untouched.
const SERVER_VEHICLE_SPECIAL = {
  racer:'rocket', tank:'cannon', intercep:'pulse', armored:'mine',
  muscle:'shotgun', pickup:'mortar', beetle:'tesla', lambo:'railgun'
};
const SERVER_BOT_WEAPON_DAMAGE = {
  gun:0.24, rocket:0.60, cannon:0.56, pulse:0.42, mine:0.46,
  shotgun:0.44, mortar:0.58, tesla:0.40, railgun:0.54
};
const SERVER_BOT_WEAPON_CD = {
  gun:[0.9,1.9], rocket:[1.8,2.8], cannon:[2.2,3.2], pulse:[1.5,2.4],
  mine:[2.5,3.6], shotgun:[1.8,2.8], mortar:[2.5,3.7], tesla:[2.4,3.5], railgun:[2.8,4.0]
};
function enemyVisualVeh(e){
  if (e.boss) return e.bossType==='artillery' ? 'pickup' : e.bossType==='commander' ? 'lambo' : 'armored';
  return e.role==='shooter' ? 'muscle' : e.role==='flanker' ? 'lambo' : e.role==='charger' ? 'pickup' : 'beetle';
}
function enemySpecialWeapon(e){ return SERVER_VEHICLE_SPECIAL[enemyVisualVeh(e)] || 'rocket'; }
function enemyCombatProfile(e){
  const sp=enemySpecialWeapon(e);
  const profiles={
    rocket:  { min:26, ideal:48, max:86, fireMin:18, fireMax:96, aim:0.62, strafe:18, specialChance:0.58 },
    cannon:  { min:30, ideal:58, max:96, fireMin:24, fireMax:108, aim:0.42, strafe:14, specialChance:0.50 },
    pulse:   { min:20, ideal:42, max:78, fireMin:12, fireMax:86, aim:0.55, strafe:22, specialChance:0.60 },
    mine:    { min:10, ideal:20, max:34, fireMin:0,  fireMax:26, aim:1.40, strafe:18, specialChance:0.38 },
    shotgun: { min:12, ideal:22, max:34, fireMin:7,  fireMax:36, aim:0.48, strafe:16, specialChance:0.66 },
    mortar:  { min:50, ideal:78, max:118,fireMin:38, fireMax:128,aim:0.78, strafe:20, specialChance:0.55 },
    tesla:   { min:8,  ideal:17, max:23, fireMin:0,  fireMax:24, aim:1.50, strafe:14, specialChance:0.72 },
    railgun: { min:34, ideal:72, max:128,fireMin:26, fireMax:138,aim:0.30, strafe:18, specialChance:0.56 }
  };
  const p={...(profiles[sp]||profiles.rocket)};
  if(e && e.boss){ p.min*=0.92; p.ideal*=1.05; p.max*=1.08; p.fireMax*=1.08; p.specialChance+=0.08; }
  if(e && e.bossType==='artillery'){ p.min=Math.max(p.min,56); p.ideal=Math.max(p.ideal,82); p.max=Math.max(p.max,124); p.fireMax=Math.max(p.fireMax,132); }
  return p;
}
function angleDiff(a,b){ let d=a-b; while(d>Math.PI)d-=Math.PI*2; while(d<-Math.PI)d+=Math.PI*2; return d; }
function chooseEnemyWeapon(e, len, aimDiff=0){
  const sp=enemySpecialWeapon(e), prof=enemyCombatProfile(e);
  let chance=e.bossType==='artillery' ? 0.68 : e.boss ? 0.52 : prof.specialChance;
  const inBand=len>=prof.fireMin && len<=prof.fireMax && Math.abs(aimDiff)<=prof.aim;
  if(!inBand) chance*=0.18;
  if (sp==='mine') chance = len<prof.fireMax ? chance : 0.03;
  else if (sp==='shotgun') chance = len<prof.fireMax ? chance : 0.04;
  else if (sp==='tesla') chance = len<=prof.fireMax ? chance : 0.00;
  else if (sp==='railgun') chance = (len>=prof.fireMin && Math.abs(aimDiff)<=prof.aim) ? chance : 0.03;
  return Math.random()<chance ? sp : 'gun';
}
function enemyWeaponCooldown(w, rateMul){
  const r=SERVER_BOT_WEAPON_CD[w] || SERVER_BOT_WEAPON_CD.gun;
  return rand(r[0], r[1]) * rateMul;
}
function enemyWeaponDamageMul(w){ return SERVER_BOT_WEAPON_DAMAGE[w] || SERVER_BOT_WEAPON_DAMAGE.gun; }
...
function desiredMoveForEnemy(lobby, e, target, dt){
  const tx0=target.client.x-e.x, tz0=target.client.z-e.z;
  const [nx,nz,len]=normalize2(tx0,tz0);
  const px=-nz, pz=nx;
  e.strafeCd-=dt;
  if (e.strafeCd<=0){ e.strafeCd=rand(1.0,3.0); if (Math.random()<0.55) e.strafeSide*=-1; }

  const prof=enemyCombatProfile(e), special=enemySpecialWeapon(e);
  const side=e.strafeSide || 1;
  const weave=Math.sin((Date.now()*0.001)*(0.75+(e.boss?0.12:0.22)) + side*1.7 + (e.id||'').length);
  const orbit=prof.strafe*(0.72+0.28*Math.sin(Date.now()*0.00037 + side));
  let goalX=target.client.x, goalZ=target.client.z, reverse=false;

  if(special==='mine'){
    goalX=target.client.x+px*side*(orbit*0.75)+nx*8;
    goalZ=target.client.z+pz*side*(orbit*0.75)+nz*8;
  } else if(special==='shotgun' || special==='tesla'){
    goalX=target.client.x+px*side*(orbit*0.55+weave*5);
    goalZ=target.client.z+pz*side*(orbit*0.55+weave*5);
    if(len<prof.min) reverse=true;
  } else if(special==='mortar' || e.bossType==='artillery'){
    goalX=target.client.x+px*side*(orbit+weave*8)-nx*18;
    goalZ=target.client.z+pz*side*(orbit+weave*8)-nz*18;
    if(len<prof.min) reverse=true;
  } else if(special==='railgun' || special==='cannon'){
    goalX=target.client.x+px*side*(orbit*0.85+weave*6)-nx*6;
    goalZ=target.client.z+pz*side*(orbit*0.85+weave*6)-nz*6;
    if(len<prof.min) reverse=true;
  } else {
    goalX=target.client.x+px*side*(orbit+weave*7);
    goalZ=target.client.z+pz*side*(orbit+weave*7);
    if(len<prof.min) reverse=true;
  }

  if(e.boss && e.bossType==='commander'){
    goalX=target.client.x+px*side*(24+Math.sin(Date.now()*0.001)*12);
    goalZ=target.client.z+pz*side*(24+Math.cos(Date.now()*0.001)*12);
  } else if(e.role==='flanker' && special!=='shotgun' && special!=='tesla'){
    goalX=target.client.x+px*side*(prof.strafe+10+Math.sin(Date.now()*0.001)*10);
    goalZ=target.client.z+pz*side*(prof.strafe+10+Math.cos(Date.now()*0.001)*10);
  } else if(e.role==='charger' || e.bossType==='ram'){
    if(e.chargeCd<=0 && len<55){ e.chargeCd=e.boss?2.1:2.7; e.chargeTime=e.boss?1.05:0.72; }
  }

  if(reverse){
    goalX=e.x-nx*34+px*side*(10+weave*5);
    goalZ=e.z-nz*34+pz*side*(10+weave*5);
  }

  let mx=goalX-e.x, mz=goalZ-e.z;
  if(len<prof.ideal-6 && (special==='mortar' || e.bossType==='artillery')){ mx=-nx; mz=-nz; }
  ...
  return { x:outX, z:outZ, len, nx, nz, prof, special, reverse };
}
...
function tickSurvival(lobbyId){
  ...
  for (const e of lobby.enemies.values()){
    ...
    const len=move.len;
    if(e.chargeTime>0) e.chargeTime=Math.max(0,e.chargeTime-dt);

    // Drive model made closer to campaign bots: the server bot now turns its car body first
    // and moves through its forward axis instead of sliding sideways like a floating puck.
    const aimYaw=angleToYaw(move.nx, move.nz);
    const movingYaw=angleToYaw(move.x, move.z);
    const inCombatRange=len < (move.prof.max+18);
    const aimOffset=inCombatRange ? ((len<move.prof.min ? 0 : (e.strafeSide||1)*0.16 + Math.sin(now*0.00115)*0.13)) : 0;
    const targetYaw=inCombatRange ? aimYaw+aimOffset : movingYaw;
    const turn=(e.boss?1.25:2.05)+(e.role==='flanker'?0.45:0)+(inCombatRange?0.35:0);
    e.yaw += Math.max(-turn*dt, Math.min(turn*dt, angleDiff(targetYaw,e.yaw)));

    const tooClose=len<move.prof.min, tooFar=len>move.prof.max;
    const desired=move.prof.ideal || e.preferredRange || 18;
    const wantsMove=tooFar || e.role==='flanker' || e.role==='charger' || e.boss || Math.abs(len-desired)>5;
    let targetSpeed;
    if(e.chargeTime>0) targetSpeed=e.speed*(e.boss?1.65:1.42);
    else if(inCombatRange && tooClose) targetSpeed=-Math.max(8,e.speed*0.42);
    else if(inCombatRange && !tooFar) targetSpeed=e.speed*(0.22+0.10*Math.sin(now*0.0009));
    else targetSpeed=wantsMove ? e.speed : 0;
    const accel=Math.min(1,dt*(e.role==='charger'?2.8:2.0));
    e.driveSpeed=(e.driveSpeed||0)+(targetSpeed-(e.driveSpeed||0))*accel;

    const oldX=e.x, oldZ=e.z;
    e.x += Math.sin(e.yaw)*e.driveSpeed*dt;
    e.z += Math.cos(e.yaw)*e.driveSpeed*dt;
    resolveArenaAndWalls(e);
    e.vx=(e.x-oldX)/Math.max(0.001,dt);
    e.vz=(e.z-oldZ)/Math.max(0.001,dt);
    if(Math.hypot(e.x-oldX,e.z-oldZ)<0.025 && Math.abs(e.driveSpeed)>5){ e.driveSpeed=-Math.max(8,Math.abs(e.driveSpeed)*0.55); e.strafeSide*=-1; }
    ...
    // Дальний бой по той же логике, что в кампании: спец-оружие стреляет только когда дистанция
    // и угол подходят под машину, иначе бот пользуется базовым пулемётом.
    const aimDiff=Math.abs(angleDiff(aimYaw,e.yaw));
    const gunRange=Math.max(62,Math.min(96,move.prof.fireMax));
    const canGun=len<gunRange && aimDiff<0.82;
    const canSpecial=len>=move.prof.fireMin && len<=move.prof.fireMax && aimDiff<move.prof.aim;
    if (e.fireCd <= 0 && (canGun || canSpecial)){
      const w=chooseEnemyWeapon(e, len, aimDiff);
      const rateMul=roleStats(e.role, e.bossType).fire || 1;
      e.fireCd = enemyWeaponCooldown(w, rateMul);
      const fx=Math.sin(aimYaw), fz=Math.cos(aimYaw);
      const ox=e.x+fx*((e.radius||BOT_RADIUS)+1.2), oy=1.78, oz=e.z+fz*((e.radius||BOT_RADIUS)+1.2);
      lobbyBroadcast(lobby,{ type:'enemyShot', id:e.id, w, x:ox, y:oy, z:oz, dx:fx, dy:0, dz:fz });

      // Серверный урон остаётся прежним по жёсткости: меняется поведение/выбор оружия, не общий дамаг.
      const accuracy=e.role==='shooter' ? 0.55 : e.boss ? 0.48 : 0.28;
      if (len < (move.prof.fireMax||65)*0.82 && Math.random()<accuracy){
        const amount = e.damage*enemyWeaponDamageMul(w);
...
function finishSurvival(lobby, win){
  ...
  for (const s of lobby.members){
    const c=clients.get(s); if (!c) continue;
    c.alive=true; c.deathHandled=false; c.activeInWave=false; c.waitingNextWave=false; c.hp=1;
  }
  lobby.state='lobby';
...
function handle(socket, client, raw){
  ...
  switch (m.type){
    ...
    case 'start': { touchClient(client); const lobby=lobbies.get(client.lobbyId);
      if (lobby && lobby.host===client.id){
        if (lobby.mode==='survival'){
          ...
          for (const s of lobby.members){ const c=clients.get(s); c.kills=0; c.deaths=0; c.alive=true; c.deathHandled=false; c.waitingNextRound=false; }
    ...
    case 'state': { const lobby=lobbies.get(client.lobbyId);
      ...
      if (client.hp>0){ client.alive=true; client.deathHandled=false; }
    ...
    case 'shot': { const lobby=lobbies.get(client.lobbyId); if (lobby && !(lobby.mode==='rounds' && client.waitingNextRound))
      lobbyBroadcast(lobby,{ type:'shot', id:client.id, w:m.w, x:m.x, y:m.y, z:m.z, dx:m.dx, dy:m.dy, dz:m.dz }, socket); break; }

    case 'damage': { const lobby=lobbies.get(client.lobbyId); if (!lobby) break;
      ...
      if (lobby.mode==='survival') handleSurvivalDamage(socket, client, lobby, m);
        ...
        if (target.hp<=0) registerPlayerDeath(lobby, target, client.id);
      }
      break; }

    case 'death': { const lobby=lobbies.get(client.lobbyId); if (!lobby) break;
      if (lobby.mode==='rounds' && client.waitingNextRound) break;
      registerPlayerDeath(lobby, client, m.killer);
      break;
    }

    case 'chat': { const lobby=lobbies.get(client.lobbyId);
...
setInterval(()=>{
  ...
  for (const [sock,client] of [...clients]){
    ...
    if (!lobby || lobby.state!=='playing') continue;
    if ((now - (client.lastActivity||now)) > AFK_LIMIT_MS){
      lobbyBroadcast(lobby,{ type:'afkNotice', id:client.id, name:client.name });
      send(sock,{ type:'afkKick', textRu:'Вас кикнуло за AFK: вы не двигались больше 1 минуты. Закройте окно и зайдите в лобби заново.', textEn:'Kicked for AFK: you did not move for more than 1 minute. Close this message and rejoin a lobby.' });
      setTimeout(()=>closeClient(sock), 250);
    }
  }
}, AFK_CHECK_MS);
