...
// Lightweight server-side filter for CrazyGames/basic moderation.
// It masks profanity, illegal-drug offers, extremist/violent terms and common obfuscated variants.
const CHAT_BLOCK_RE = [
  /\b(cocaine|coke|heroin|meth|weed|marijuana|lsd|mdma|ecstasy|opioid|fentanyl|drug[s]?|sell\s*drug[s]?|buy\s*drug[s]?)\b/gi,
  /\b(nazi|hitler|terrorist|bomb\s*making|kill\s*yourself|kys)\b/gi,
  /\b(fuck|shit|bitch|asshole|bastard|dick|pussy|cunt|slut|whore|retard)\b/gi,
...
const CHAT_HARD_TOKENS = [
  'cocaine','coke','heroin','meth','weed','marijuana','fentanyl','drug','selldrugs','buydrugs',
  'nazi','hitler','terrorist','bombmaking','killyourself','kys',
  'fuck','shit','bitch','asshole','bastard','cunt','slut','whore',
  'кокаин','героин','метамфетамин','марихуана','наркотик','наркота','продамнарк','куплюнарк',
...
function filterChatText(text){
  ...
  const norm=normalizeChatForFilter(raw);
  if (CHAT_HARD_TOKENS.some(w=>norm.includes(w))) { changed=true; t='####'; }
...
function handle(socket, client, raw){
  ...
  switch (m.type){
    ...
    case 'damage': { const lobby=lobbies.get(client.lobbyId); if (!lobby) break;
      ...
      else lobbyBroadcast(lobby,{ type:'damage', from:client.id, target:m.target, amount:m.amount }, socket);
      break; }
