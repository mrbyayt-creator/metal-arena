...
function vehHp(veh){ return VEHICLE_HP[veh] || 100; }
function vehArmor(veh){ return VEHICLE_ARMOR[veh] || 1; }
...
function onConnect(socket, head){
  const client = {
    ...
    x:0, z:0, yaw:0, hp:1, hpLockUntil:0
  };
...
function handle(socket, client, raw){
  ...
  switch (m.type){
    ...
    case 'state': { const lobby=lobbies.get(client.lobbyId);
      client.x=Number(m.x)||0; client.z=Number(m.z)||0; client.yaw=Number(m.yaw)||0;
      const incomingHp=Math.max(0,Math.min(1,Number(m.hp)||0));
      if (Date.now() < (client.hpLockUntil||0) && incomingHp > client.hp) client.hp=client.hp;
    ...
    case 'damage': { const lobby=lobbies.get(client.lobbyId); if (!lobby) break;
      ...
      if (lobby.mode==='survival') { handleSurvivalDamage(socket, client, lobby, m); break; }
      const targetSock=socketById(Number(m.target));
      const target=targetSock?clients.get(targetSock):null;
      if (!target || target.lobbyId!==lobby.id || target.id===client.id) break;
      if ((client.team===0||client.team===1) && client.team===target.team) break;
      const raw=Math.max(0,Math.min(120,Number(m.amount)||0));
      const finalDamage=raw*vehArmor(target.veh);
      const maxHp=vehHp(target.veh);
      target.hp=Math.max(0,Math.min(1,(Number(target.hp)||0) - finalDamage/Math.max(1,maxHp)));
      target.hpLockUntil=Date.now()+500;
      if (target.hp<=0) target.alive=false;
      lobbyBroadcast(lobby,{ type:'damage', from:client.id, target:target.id, amount:finalDamage, rawAmount:raw, hp:target.hp });
      break; }
