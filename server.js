...
const server = http.createServer((req, res) => {
  ...
  if (urlPath === '/ping' || urlPath === '/health') {
    ...
    res.end(JSON.stringify({ ok:true, ts:Date.now(), players:clients.size, lobbies:lobbies.size }));
    return;
  }
  ...
  fs.readFile(full, (err, data) => {
    ...
    const types = { '.html':'text/html; charset=utf-8', '.js':'text/javascript', '.css':'text/css', '.mp3':'audio/mpeg', '.json':'application/json; charset=utf-8' };
...
// AFK counts only the player's own driving input. Shots, chat, damage, and being pushed do not reset it.
function touchClient(client){ /* network activity is not AFK activity anymore */ }
function touchClientByMotion(client, x, z, yaw, activeMove){
  if(!client) return;
  client.lastMoveX = x||0;
  client.lastMoveZ = z||0;
  client.lastMoveYaw = yaw||0;
  if(activeMove) client.lastActivity = Date.now();
}
...
function createLobby(socket, client, m, forcedMode){
  ...
  lobbyBroadcast(lobby,{ type:'playerJoined', id:client.id, name:client.name, team:client.team });
...
function startSurvival(lobby){
  ...
  for (const s of lobby.members){
    const c=clients.get(s);
    c.kills=0; c.deaths=0; c.alive=true; c.deathHandled=false; c.team=null; c.activeInWave=true; c.waitingNextWave=false; c.hp=1; c.lastActivity=Date.now();
  }
...
function tickSurvival(lobbyId){
  ...
  for (const e of lobby.enemies.values()){
    ...
    // Как в кампании: в боевой дистанции держит перед машины/оружие в сторону игрока,
    // а не уезжает спиной.
    const aimYaw=angleToYaw(move.nx, move.nz);
    const movingYaw=Math.hypot(e.vx,e.vz)>0.4 ? angleToYaw(e.vx,e.vz) : e.yaw;
    const inCombatRange=len < (move.prof.max+18);
    const aimOffset=inCombatRange ? ((len<move.prof.min ? 0 : (e.strafeSide||1)*0.16)) : 0;
    const targetYaw=inCombatRange ? aimYaw+aimOffset : movingYaw;
    e.yaw += Math.max(-2.1*dt, Math.min(2.1*dt, angleDiff(targetYaw,e.yaw)));
...
function handle(socket, client, raw){
  ...
  switch (m.type){
    ...
    case 'start': { touchClient(client); const lobby=lobbies.get(client.lobbyId);
      if (lobby && lobby.host===client.id){
        if (lobby.mode==='survival'){
          ...
          for (const s of lobby.members){ const c=clients.get(s); c.kills=0; c.deaths=0; c.alive=true; c.deathHandled=false; c.waitingNextRound=false; c.lastActivity=Date.now(); }
    ...
    case 'state': { const lobby=lobbies.get(client.lobbyId);
      client.x=Number(m.x)||0; client.z=Number(m.z)||0; client.yaw=Number(m.yaw)||0; touchClientByMotion(client, client.x, client.z, client.yaw, !!m.move);
...
setInterval(()=>{
  ...
  for (const [sock,client] of [...clients]){
    ...
    if ((now - (client.lastActivity||now)) > AFK_LIMIT_MS){
      if (lobby) lobbyBroadcast(lobby,{ type:'afkNotice', id:client.id, name:client.name });
      send(sock,{ type:'afkKick' });
