...
// ---------- Strong local chat moderation ----------
// Server-side filter for CrazyGames/basic moderation: masks profanity/drugs and blocks links/contact sharing.
const CHAT_BLOCK_RE = [
  /\b(cocaine|coke|heroin|meth|weed|marijuana|lsd|mdma|ecstasy|opioid|fentanyl|drug[s]?|sell\s*drug[s]?|buy\s*drug[s]?)\b/gi,
  /\b(nazi|hitler|terrorist|bomb\s*making|kill\s*yourself|kys|suicide)\b/gi,
  /\b(fuck|shit|bitch|asshole|bastard|dick|pussy|cunt|slut|whore|retard|nigger|nigga|faggot)\b/gi,
  /(сука|сучка|бля+|блять|блядь|еба+|ебан|ебать|нахуй|пизд|хуй|хуе|мудак|гандон|долбоеб|долбоёб|мразь|шлюха|чмо|тварь)/gi,
  /(кокаин|героин|мет|метамфетамин|марихуана|травка|наркотик|наркота|продам\s*нарк|куплю\s*нарк)/gi
];
const CHAT_CONTACT_RE = [
  /https?:\/\//i, /\bwww\./i, /\b(t\.me|telegram\.me|discord\.gg|discord\.com\/invite|wa\.me|vk\.com|instagram\.com|youtube\.com|youtu\.be)\b/i,
  /@[a-z0-9_]{3,}/i,
  /[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/i,
  /(?:\+?\d[\s().-]*){7,}/
];
const CHAT_HARD_TOKENS = [
  'cocaine','coke','heroin','meth','weed','marijuana','lsd','mdma','ecstasy','opioid','fentanyl','drug','selldrugs','buydrugs',
  'nazi','hitler','terrorist','bombmaking','killyourself','suicide','kys',
  'fuck','shit','bitch','asshole','bastard','dick','pussy','cunt','slut','whore','retard','nigger','nigga','faggot',
  'кокаин','героин','метамфетамин','марихуана','травка','наркотик','наркота','продамнарк','куплюнарк',
  'блять','блядь','ебать','ебан','нахуй','пизд','хуй','мудак','гандон','долбоеб','долбоёб','мразь','шлюха','сука','чмо','тварь'
];
const CHAT_CONTACT_TOKENS = [
  'telegram','телеграм','телега','тг','tg','discord','дискорд','whatsapp','ватсап','viber','вайбер','instagram','insta','инста','vk','вк','snapchat','skype','скайп','phone','телефон','номер','contactme','addme','writeinpm','напишивлс','пишивлс'
];
function squashRepeats(s){ return s.replace(/([a-zа-яё0-9])\1{1,}/gi,'$1'); }
function normalizeChatVariants(text){
  const base=(text||'').toString().toLowerCase()
    .normalize('NFKD').replace(/[\u0300-\u036f]/g,'')
    .replace(/[0]/g,'o').replace(/[1!|]/g,'i').replace(/[3]/g,'e').replace(/[4]/g,'a').replace(/[5$]/g,'s').replace(/[7]/g,'t');
  const compact=squashRepeats(base.replace(/[^a-zа-яё0-9]+/gi,''));
  const cyr=squashRepeats(base
    .replace(/[a@]/g,'а').replace(/[e]/g,'е').replace(/[o]/g,'о').replace(/[p]/g,'р').replace(/[c]/g,'с')
    .replace(/[x]/g,'х').replace(/[y]/g,'у').replace(/[k]/g,'к').replace(/[m]/g,'м').replace(/[t]/g,'т')
    .replace(/[^a-zа-яё0-9]+/gi,''));
  return [compact,cyr];
}
function normalizeChatForFilter(text){ return normalizeChatVariants(text)[0]; }
function filterChatText(text){
  const raw=(text||'').toString().slice(0,120);
  let t=raw, changed=false;
  const variants=normalizeChatVariants(raw);
  const hasContact = CHAT_CONTACT_RE.some(re=>re.test(raw)) || CHAT_CONTACT_TOKENS.some(w=>variants.some(v=>v.includes(w)));
  if (hasContact) return { text:'', changed:true, blocked:true };
  for (const re of CHAT_BLOCK_RE) t=t.replace(re, m=>{ changed=true; return '#'.repeat(Math.min(12, Math.max(4, m.length))); });
  if (CHAT_HARD_TOKENS.some(w=>variants.some(v=>v.includes(w)))) { changed=true; t='####'; }
  return { text:t, changed, blocked:false };
}
...
function handle(socket, client, raw){
  ...
  switch (m.type){
    ...
    case 'leaveLobby': removeFromLobby(socket); send(socket, lobbyListPayload()); break;

    case 'team': { const lobby=lobbies.get(client.lobbyId); if (lobby){ const ok=setTeam(lobby, client, m.team);
    ...
    case 'start': { const lobby=lobbies.get(client.lobbyId);
    ...
    case 'state': { const lobby=lobbies.get(client.lobbyId);
      client.x=Number(m.x)||0; client.z=Number(m.z)||0; client.yaw=Number(m.yaw)||0; client.hp=Number(m.hp)||0;
      ...
      if (lobby) lobbyBroadcast(lobby,{ type:'state', id:client.id, x:m.x, z:m.z, yaw:m.yaw, hp:m.hp, team:client.team }, socket);
      break;
    }
    ...
    case 'damage': { const lobby=lobbies.get(client.lobbyId); if (!lobby) break;
      ...
      else lobbyBroadcast(lobby,{ type:'damage', from:client.id, target:m.target, amount:m.amount });
      break; }
    ...
    case 'chat': { const lobby=lobbies.get(client.lobbyId);
      if (lobby){ const filtered=filterChatText(m.text); if(filtered.blocked || !filtered.text.trim()){ send(socket,{type:'chatBlocked'}); break; } lobbyBroadcast(lobby,{ type:'chat', id:client.id, name:client.name, team:client.team, text:filtered.text }, socket); }
      break; }
  }
}
...
function closeClient(socket){
  ...
  console.log(`- ${client.name||'Игрок'} (#${client.id}) отключился`);
