...
function cleanPlayerName(name){ return (name||'Игрок').toString().trim().slice(0,14) || 'Игрок'; }
...
function createLobby(socket, client, m, forcedMode){
  ...
  if (nameTaken(client.name, socket, true)){
    send(socket,{ type:'nameTaken', name:client.name, text:'Такой ник уже занят. Поменяй никнейм.' });
    return;
  }
  ...
  const lobby={
    id:nextLobby++,
    name:(m.name||('Лобби '+client.name)).toString().slice(0,20),
    host:client.id,
...
function handle(socket, client, raw){
  ...
  switch (m.type){
    ...
    case 'joinLobby': {
      ...
      if (nameTaken(client.name, socket, true)){ send(socket,{ type:'nameTaken', name:client.name, text:'Такой ник уже занят. Поменяй никнейм.' }); break; }
    ...
    case 'chat': { const lobby=lobbies.get(client.lobbyId);
      if (lobby) lobbyBroadcast(lobby,{ type:'chat', id:client.id, name:client.name, team:client.team, text:(m.text||'').toString().slice(0,120) }, socket);
      break; }
  }
}
